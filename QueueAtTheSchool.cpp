...
Initially the children stood in the order they entered the canteen. However, 
after a while the boys started feeling awkward for standing in front of the girls in the queue 
...
at that the person in the position number 1 is served first. Then, if at time x a boy stands on the i-th position 
and a girl stands on the (i + 1)-th position, then at time x + 1 the i-th position will have a girl and the (i + 1)-th position will have a boy. 
...
int main()
{
    int standing, time;
    string who;
    cin >> standing >> time >> who;


    for(int j = 0; j < time; j++){
        for(int i = 0; i < standing; i++){
            if(who[i] == 'B' && who[i + 1] == 'G'){
                who[i + 1] = who[i];
                who[i] = 'G';
                i++;
            }
