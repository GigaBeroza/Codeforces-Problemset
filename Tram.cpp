...
and the number of passengers that enter the tram at the i-th stop. The stops are given from the first to the last stop 
in the order of tram's movement.
...
int main()
{
    ...
    for(int i = 0; i < stop; i++){
        ...
        if(i < 1){
                ...
                capacity = capacity + train[i][1];
            }else if(i > 0){
                capacity = capacity - train[i][0];
                capacity = capacity + train[i][1];
            }
        if(capacity > full){
            full = capacity;
